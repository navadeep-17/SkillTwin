    ...
    await sql.begin(async tx => {
      ...
      for (const task of oldTasks) {
        const inserted = rows(await tx.unsafe(
          ...
          [
            nextPlanId,
            weekMap.get(String(task.week_id)),
            objectiveMap.get(String(task.objective_id)),
